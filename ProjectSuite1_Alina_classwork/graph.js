...
function graph_regenerateGraph(element)
{
	...
	if (abscissColumn != null && ordinateColumn != null)
	{
		...
		var rows = _select_Nodes(element.table.xmlData.documentElement, element.table.getPath() + "[@pid=\"-1\"]");
		var _skipCategories = Math.round(rows.length / 10);
		if (_skipCategories < 1) _skipCategories = 1;
		for (var i = 0; i < rows.length; i++)
		{
			var oRow = rows[i];
			var columns = _select_Nodes(oRow, abscissColumn.name);
			...
			columns = _select_Nodes(oRow, ordinateColumn.name);
